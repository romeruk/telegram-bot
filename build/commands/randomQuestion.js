...
var randomQuestion = function randomQuestion(bot) {
  bot.command('question', (0, _index.limitsMiddleware)(), function (ctx) {
    var randomQuestion =
      _arrayOfQuestions.questions[
        Math.floor(Math.random() * _arrayOfQuestions.questions.length)
      ];
    ...
    try {
      for (
        var _iterator = randomQuestion.answerOptions
            .entries()
            [Symbol.iterator](),
          _step;
  ...
  for (
    var _i2 = 0, _Object$entries = Object.entries(_arrayOfQuestions.variants);
